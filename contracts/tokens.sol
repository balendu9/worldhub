...
contract Tokens is ERC20 {
  ...
  function buy() external payable {
    uint256 paymentAmount = msg.value;  // The ETH sent by the buyer
    ...
    _mint(msg.sender, tokenAmount);  // Minting the tokens to the buyer
  ...
  function sell(uint256 tokenAmount) external {
    ...
    _burn(msg.sender, tokenAmount);  // Burn the tokens the user is selling
    userWithdrawn[msg.sender] += ethAmount;

    // Transfer the calculated ETH amount back to the seller
    payable(msg.sender).transfer(ethAmount);  // Sending ETH to the seller
